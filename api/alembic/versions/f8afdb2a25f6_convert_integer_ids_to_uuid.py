...
def upgrade() -> None:
    ...
    op.add_column('mcp_servers', sa.Column('id_uuid', postgresql.UUID(as_uuid=True), nullable=True))
    ...
    op.add_column('mcp_server_tokens', sa.Column('mcp_server_id_uuid', postgresql.UUID(as_uuid=True), nullable=True))
    ...
    # Add new UUID column for server reference
    op.add_column('mcp_server_tools', sa.Column('mcp_server_id_uuid', postgresql.UUID(as_uuid=True), nullable=True))
    ...
    op.drop_constraint('mcp_server_tools_mcp_server_id_fkey', 'mcp_server_tools', type_='foreignkey')
    op.drop_constraint('mcp_server_tokens_mcp_server_id_fkey', 'mcp_server_tokens', type_='foreignkey')
    ...
    op.alter_column('mcp_server_tokens', 'mcp_server_id_uuid', new_column_name='mcp_server_id')
    op.create_foreign_key(
        'mcp_server_tokens_mcp_server_id_fkey',
        'mcp_server_tokens', 'mcp_servers',
        ['mcp_server_id'], ['id'],
        ondelete='CASCADE'
    )
    ...
    op.alter_column('mcp_server_tools', 'mcp_server_id_uuid', new_column_name='mcp_server_id')
    op.create_foreign_key(
...
def downgrade() -> None:
    ...
    op.add_column('mcp_servers', sa.Column('id', sa.Integer(), nullable=False, server_default=sa.text("nextval('mcp_servers_id_seq'::regclass)")))
    op.create_primary_key('mcp_servers_pkey', 'mcp_servers', ['id'])
    
    # mcp_server_tokens
    op.alter_column('mcp_server_tokens', 'mcp_server_id', new_column_name='mcp_server_id_uuid')
    op.add_column('mcp_server_tokens', sa.Column('mcp_server_id', sa.Integer(), nullable=True))
    
    # mcp_server_tools
    op.alter_column('mcp_server_tools', 'mcp_server_id', new_column_name='mcp_server_id_uuid')
    op.add_column('mcp_server_tools', sa.Column('mcp_server_id', sa.Integer(), nullable=True))
