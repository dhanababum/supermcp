...
from datatypes import ToolType, ConnectorMode

from fastapi_users_db_sqlalchemy import SQLAlchemyBaseUserTableUUID
from sqlalchemy import Column, String, Boolean, DateTime, Table, Enum as SQLEnum, LargeBinary
...
class McpConnector(SQLModel, table=True):
    ...
    is_active: bool = Field(default=True, description="Whether this connector record is active (soft delete)")
...
class ConnectorAccess(SQLModel, table=True):
    ...
    connector_id: Optional[uuid.UUID] = Field(
        default=None,
        sa_column=Column(UUID(as_uuid=True), ForeignKey('mcp_connectors.id', ondelete='CASCADE')),
...
class McpServerToken(SQLModel, table=True):
    ...
    mcp_server_id: Optional[uuid.UUID] = Field(
        default=None,
        sa_column=Column(UUID(as_uuid=True), ForeignKey('mcp_servers.id', ondelete='CASCADE')),
